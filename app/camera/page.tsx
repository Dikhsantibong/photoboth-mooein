            ...
            <div className="flex-1 flex flex-col min-h-0 min-w-0 gap-2">
              ...
              <div className="flex-1 min-h-0 bg-white rounded-[20px] shadow-2xl p-4 flex flex-col items-center relative overflow-hidden border-3 border-white gap-4">
                ...
                <div className="flex-1 min-h-0 w-full flex items-center justify-center relative">
                        ...
                        return (
                          ...
                          >
                            {isCurrent && !showPreview ? (
                              ...
                              <span className="text-black/30 font-black text-4xl">{i + 1}</span>
                            )}
                          </div>
                        );
                      })}
                  ...
                  {/* Countdown */}
                  {countdown !== null && (
                    <div className="absolute inset-0 flex items-center justify-center z-30 bg-black/20 rounded-[20px]">
                      <span className={`${poppins.className} text-[6rem] lg:text-[8rem] text-white drop-shadow-2xl animate-ping font-black`}>{countdown}</span>
                    </div>
                  )}

                  {/* Flash Overlay */}
                  {flashActive && <div className="absolute inset-0 bg-white z-50 animate-out fade-out duration-300 rounded-[20px]"></div>}
                </div>

                {/* Camera Buttons Row Inside Card (Stacked below template) */}
                <div className="shrink-0 flex items-center justify-center gap-4 w-full">
                  {!showPreview ? (
                    <>
                      <button
                        ...
                        className={`w-14 h-14 rounded-full backdrop-blur-md flex items-center justify-center shadow-lg transition-all active:scale-95 border-3
                          ${isMirrored ? "bg-black text-white border-white" : "bg-white/90 text-black border-black"}`}
                        title="Mirror Camera"
                      >
